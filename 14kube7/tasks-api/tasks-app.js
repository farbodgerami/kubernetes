...
const filePath ='/app/tasks/tasks.txt'
const app = express();
...
const extractAndVerifyToken = async (headers) => {
  ...
  const response = await axios.get(`http://${process.env.AUTHADDRESS}/verify-token/` + token);
...
app.get('/tasks', async (req, res) => {
  try {
    ...
    fs.readFile(filePath, (err, data) => {
      if (err) {
        console.log(err);
        return res.status(500).json({ message: 'nisssssssssssssssst' });
      }
