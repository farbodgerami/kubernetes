...
app.post('/signup', async (req, res) => {
  ...
  try {
    // vase vasl kardane pode users be pode aut az esme service neveshte shode dar authservice.yaml ro be tore capital darim:
    const hashedPW = await axios.get(`http://${process.env.AUTHADDRESS}/hashed-password/` + password);
...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get(
    `http://${process.env.AUTHSSERVICE_SERVICE_HOST}/token/` + hashedPassword + '/' + password
  );
